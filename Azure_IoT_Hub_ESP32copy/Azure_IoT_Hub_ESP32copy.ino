...
int Threshold = 550;      // Signal threshold for beat detection
PulseSensorPlayground pulseSensor;  // Creates an instance of the PulseSensorPlayground object

// MPU6050 wiring and address configurations
MPU6050 mpu68(0x68);  // Device 0x68
MPU6050 mpu69(0x69);  // Device 0x69

// Error code for device operations
uint8_t error_code = 0U;

// Quaternions for orientation data from the sensors
Quaternion q68;
Quaternion q69;

// Buffer for the DMP data
uint8_t fifo_buffer68[64]; // FIFO storage buffer for device 0x68
uint8_t fifo_buffer69[64]; // FIFO storage buffer for device 0x69
...
static void generateTelemetryPayload(int myBPM) {
  telemetry_payload = "{\"key\": \"BPM\", \"value\": " + String(myBPM) + "}";
}

static void sendTelemetry(int myBPM)
{
  ...
  // 生成遥测负载
  generateTelemetryPayload(myBPM);
...
void setup() {
  ...
  // PulseSensor setup
  pulseSensor.analogInput(PulseWire);  // Configure the PulseSensor object
  pulseSensor.setThreshold(Threshold);

  if (pulseSensor.begin()) {
    Serial.println("PulseSensor object created!");  // Confirm object creation
  }

  establishConnection();  // Establish WiFi and MQTT connection
}
...
void loop() {
  ...
  if (pulseSensor.sawStartOfBeat()) {  // Check if a beat is detected
    ...
    // Generate and send telemetry data with BPM
    sendTelemetry(myBPM);
  }
}
